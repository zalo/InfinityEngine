...
assetListLoader.load(async () => {
    ...
    new pc.MiniStats(app, pc.MiniStats.getDefaultOptions(['gsplats', 'gsplatsCopy'])); // eslint-disable-line no-unused-vars
    ...
    const loadGsplat = async (/** @type {string|null} */ url) => {
        ...
        gsplatEntity = new pc.Entity(config.name); // eslint-disable-line require-atomic-updates
